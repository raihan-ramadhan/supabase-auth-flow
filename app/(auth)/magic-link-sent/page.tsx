...
import ResendMagicLinkButton from "./ResendMagicLink";

export default async function AuthSuccessPage({ searchParams }: { searchParams: Promise<{ email?: string }> }) {
  const { email } = await searchParams;

  return (
    <div className="flex flex-col items-center justify-center min-h-screen">
      <svg className="w-[150px] h-auto" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 570 511.67482" role="img">
        <path
      ...
      <div className="p-6 rounded-lg shadow-md">
        <div className="flex flex-col items-center">
          <p className="text-lg font-semibold text-center max-w-sm">
        ...
        <div className="mt-4 text-center max-w-xs mx-auto">
          {email ? (
            <Suspense>
              <ResendMagicLinkButton email={email} />
            </Suspense>
          ) : (
            <span className="text-sm">
              ...
              <span className="text-gray-600 dark:text-gray-400"> to go back and try again.</span>
            </span>
          )}
        </div>
      </div>
